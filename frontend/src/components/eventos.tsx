      ...
      <div className="relative z-[1] mx-auto max-w-[1180px] px-8">
        <div className="grid grid-cols-1 items-center gap-14 rounded-sm border border-cream/12 bg-cream/[0.04] p-8 md:grid-cols-[0.85fr_1.15fr] md:gap-16 md:p-[50px]">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-[18px] border-t border-cream/15 pt-[26px]"><div className="font-serif text-[1.7rem] text-cream">{formatPrice(event.precio)}<span className="block font-sans text-[0.76rem] uppercase tracking-[0.04em] text-cream/55">Por persona · {event.duracion >= 60 ? `${event.duracion / 60} horas` : `${event.duracion} minutos`} · {event.ubicacion}</span></div><a href={`https://wa.me/573000000000?text=${encodeURIComponent(`Hola, quiero reservar mi cupo para ${event.nombre}`)}`} target="_blank" rel="noopener noreferrer" className="inline-block rounded-sm bg-gold px-8 py-4 text-sm uppercase tracking-wider text-ink transition-all duration-300 hover:-translate-y-0.5 hover:bg-gold-light">Reservar mi cupo</a></div>
          </div>
        </div>
      </div>
    </section>
