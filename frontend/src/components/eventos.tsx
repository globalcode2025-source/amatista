...
export function Eventos() {
  ...
  const displayPrice = (event: EventoAdmin) => {
    if (event.precio_descuento && event.descuento) {
      return event.precio_descuento.toLocaleString('es-CO');
    }
    return event.precio.toLocaleString('es-CO');
  };
          ...
          return (
            <div key={event.id} className="mb-16 last:mb-0 grid grid-cols-1 items-center gap-14 rounded-sm border border-cream/12 bg-cream/[0.04] p-8 md:grid-cols-[0.85fr_1.15fr] md:gap-16 md:p-[50px]">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-[18px] border-t border-cream/15 pt-[26px]"><div className="flex flex-col">
                  {event.precio_descuento && event.descuento && (
                    <span className="text-sm text-cream/40 line-through">
                      {formatPrice(event.precio)}
                    </span>
                  )}
                  <div className="font-serif text-[1.7rem] text-cream">$ {displayPrice(event)}<span className="block font-sans text-[0.76rem] uppercase tracking-[0.04em] text-cream/55">Por persona · {event.duracion >= 60 ? `${event.duracion / 60} horas` : `${event.duracion} minutos`} · {event.ubicacion}</span></div>
                </div><a href={`https://wa.me/573147325051?text=${encodeURIComponent(`Hola, quiero reservar mi cupo para ${event.nombre}`)}`} target="_blank" rel="noopener noreferrer" className="inline-block rounded-sm bg-gold px-8 py-4 text-sm uppercase tracking-wider text-ink transition-all duration-300 hover:-translate-y-0.5 hover:bg-gold-light">Reservar mi cupo</a></div>
              </div>
            </div>
          );
        })}
