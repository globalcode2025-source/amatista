...
export default function Dashboard() {
  ...
  const totalIngresos = ingresos.items.reduce((sum, i) => sum + i.monto, 0);
  const totalGastos = gastos.items.reduce((sum, g) => sum + g.monto, 0);
  const balance = totalIngresos - totalGastos;
  const pedidosPendientes = pedidos.items.filter((p) => p.estado === 'Pendiente').length;

  const proximoEvento = [...eventos.items]
    .filter((e) => e.estado === 'Próximo')
    .sort((a, b) => a.fecha.localeCompare(b.fecha))[0];

  const recientes = [...pedidos.items].sort((a, b) => b.fecha.localeCompare(a.fecha)).slice(0, 5);
