...
const formatCurrency = (value: string) => {
  ...
  return numeric ? Number(numeric).toLocaleString('es-CO') : '';
};
...
export default function EventosPage() {
  ...
  const [attendeePaymentDisplay, setAttendeePaymentDisplay] = useState('');
  ...
  const openRegistration = async () => { const availableEvent = items.find((item) => item.estado === 'Próximo' && item.cuposDisponibles > 0); setRegistrationEventId(availableEvent?.id ?? ''); setAttendeeForm(EMPTY_ATTENDEE); setAttendeePaymentDisplay(''); setClientQuery(''); setAttendeesError(''); setRegistrationOpen(true); try { setClients(await fetchClientes()); } catch (err) { setAttendeesError(err instanceof Error ? err.message : 'No se pudieron cargar los clientes'); } };
  const registerAttendee = async (event: FormEvent) => { event.preventDefault(); if (!registrationEvent || !attendeeForm.clienteId) return; try { const pagoNumerico = attendeePaymentDisplay === '' ? 0 : Number(attendeePaymentDisplay.replace(/\D/g, '')); await createAsistenteEvento(registrationEvent.id, { clienteId: attendeeForm.clienteId, pago: pagoNumerico }); setRegistrationOpen(false); await load(); } catch (err) { setAttendeesError(err instanceof Error ? err.message : 'No se pudo registrar el asistente'); } };
  ...
  return <div>
    ...
    <Modal open={registrationOpen} title="Registrar asistente" onClose={() => setRegistrationOpen(false)}><form onSubmit={registerAttendee} className="flex flex-col gap-4"><label className="block"><span className="mb-1.5 block text-xs uppercase tracking-wide text-ink/55">Evento</span><select required value={registrationEventId} onChange={(e) => setRegistrationEventId(e.target.value)} className="w-full rounded-sm border border-ink/15 bg-white px-3.5 py-2.5 text-sm"><option value="" disabled>Selecciona un evento...</option>{items.filter((item) => item.estado === 'Próximo' && item.cuposDisponibles > 0).map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></label>{registrationEvent && <p className="rounded-sm bg-cream px-3 py-2 text-sm text-ink">Precio: <strong>{registrationEvent.precio_descuento ? <><span className="line-through text-ink/40">{money(registrationEvent.precio)}</span> → {money(registrationEvent.precio_descuento)}</> : money(registrationEvent.precio)}</strong></p>}<label className="block"><span className="mb-1.5 block text-xs uppercase tracking-wide text-ink/55">Buscar cliente</span><input value={clientQuery} onChange={(e) => { setClientQuery(e.target.value); setAttendeeForm((old) => ({ ...old, clienteId: '' })); }} placeholder="Nombre, teléfono o correo" className="w-full rounded-sm border border-ink/15 bg-white px-3.5 py-2.5 text-sm" /></label>{clientQuery && !selectedClient && <div className="max-h-40 overflow-y-auto rounded-sm border border-ink/10 bg-white">{matchingClients.length === 0 ? <p className="px-3 py-2 text-sm text-ink/55">No hay clientes coincidentes.</p> : matchingClients.map((client) => <button key={client.id} type="button" onClick={() => { setAttendeeForm((old) => ({ ...old, clienteId: client.id })); setClientQuery(client.nombre); }} className="block w-full border-b border-ink/10 px-3 py-2 text-left text-sm hover:bg-cream/60"><strong>{client.nombre}</strong><span className="block text-xs text-ink/55">{client.telefono} · {client.email}</span></button>)}</div>}{selectedClient && <p className="rounded-sm bg-cream px-3 py-2 text-sm text-ink">Cliente seleccionado: <strong>{selectedClient.nombre}</strong></p>}{attendeesError && <p className="text-sm text-danger">{attendeesError}</p>}<input required type="text" inputMode="numeric" value={attendeePaymentDisplay} onChange={(e) => setAttendeePaymentDisplay(formatCurrency(e.target.value))} placeholder="Pago inicial" className="w-full rounded-sm border border-ink/15 bg-white px-3.5 py-2.5 text-sm" /><div className="flex justify-end gap-3"><button type="button" onClick={() => setRegistrationOpen(false)} className="px-5 py-2.5 text-sm text-ink/60">Cancelar</button><button type="submit" className="rounded-sm bg-gold px-6 py-2.5 text-sm text-ink">Registrar</button></div></form></Modal>
